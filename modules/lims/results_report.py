...
@app.route('/<database_name>/verify', methods=['GET'])
@with_pool
@with_transaction()
def verify(request,pool):

    report = request.args.to_dict()['report']
    Results = pool.get('lims.results_report')
    
    report = ResultsReport.search([('number', '=',report),])

    ResultReport = pool.get('lims.global_result_report', type='report')
    global_report = ResultReport.execute([report[0].id], {'english_report': True,})
   
    return Response( global_report[1], mimetype="application/pdf" )
...
class ResultsReport(ModelSQL, ModelView):
    ...
    def make_qrcode(self, number):
        Config = Pool().get('lims.configuration')
        qr_link = Config(1).qr_link 
        tr = Transaction()
        # http://127.0.0.1:8002/lerm/verify?report=REP/2021/12/14/1
        qr_string = qr_link+'/'+tr.database.name+'/verify?report='+self.number
